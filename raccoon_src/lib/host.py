...
class Host:
    ...
    def write_up(self):
        self.logger.info(
            "{} Writing DNS query results".format(COLORED_COMBOS.GOOD, self)
        )
    ...
    def parse(self):
        ...
        if self.validate_ip(self.target):
            self.logger.info(
                "{} Detected {} as an IP address.".format(
            ...
            if self.target.startswith("www."):
                ...
                if len(domain_levels) == 2 or (
                    len(domain_levels) == 3 and domain_levels[1] == "co"
                ):
                    self.logger.info(
                        "{} Found {} to be a naked domain".format(
                            COLORED_COMBOS.NOTIFY, self.target
                        )
                    )
