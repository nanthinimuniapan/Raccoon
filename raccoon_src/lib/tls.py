...
class TLSHandler(TLSCipherSuiteChecker):
    ...
    def _are_certificates_identical(self):
        ...
        if (
            all(cert for cert in (sni_cert, non_sni_cert) if cert)
            and sni_cert == non_sni_cert
        ):
            return True
        return
    ...
    async def _is_heartbleed_vulnerable(self):
        ...
        process = await create_subprocess_exec(
            *script.split(), stdout=PIPE, stderr=PIPE
        )
        result, err = await process.communicate()
        try:
    ...
    async def _execute_ssl_data_extraction(self, sni=False):
        ...
        for res in responses:
            if self._is_certificate_exists(res):
                ...
                tls_dict[
                    "Certificate_details"
                ] = await self._extract_certificate_details(res)
                break
    ...
    async def _run_openssl_sclient_cmd(self, script, sni=False):
        ...
        for v in self._versions:
            ...
            processes.append(
                await create_subprocess_exec(*curr, stdout=PIPE, stderr=PIPE)
            )
        for p in processes:
    ...
    async def _get_sans_from_openssl_cmd(data):
        process = await create_subprocess_exec(
            "openssl", "x509", "-noout", "-text", stdin=PIPE, stderr=PIPE, stdout=PIPE
        )
        result, err = await process.communicate(input=bytes(data, encoding="ascii"))
    ...
    async def run(self):
        self.logger.info(
            "{} Started collecting TLS data for {}".format(
