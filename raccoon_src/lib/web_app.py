...
class WebApplicationScanner:
    ...
    def _x_powered_by(self):
        if self.headers.get("X-Powered-By"):
            self.logger.info(
                "{} X-Powered-By header detected: {}{}{}".format(
                    COLORED_COMBOS.GOOD,
                    COLOR.GREEN,
    ...
    def _robots(self):
        res = self.request_handler.send(
            ...
            url="{}://{}:{}/robots.txt".format(
                self.host.protocol, self.host.target, self.host.port
            ),
        )
    ...
    def _analyze_hrefs(self, href):
        if all(("?" in href, "=" in href, not href.startswith("mailto:"))):
            if any(
                (
                    (self.host.naked and self.host.naked in href),
                    self.host.target in href,
                    href.startswith("/"),
                )
            ):
    ...
    def _find_urls(self, soup):
        ...
        if urls:
            ...
            if self.fuzzable_urls:
                self.logger.info(
                    "{} {} fuzzable URLs discovered".format(
            ...
            if self.emails:
                self.logger.info(
                    "{} {} email addresses discovered".format(
    ...
    async def run_scan(self):
        self.logger.info(
            "{} Trying to collect {} web application data".format(
