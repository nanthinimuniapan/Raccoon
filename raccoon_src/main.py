...
@click.option(
    "-d",
    "--dns-records",
    default="A,MX,NS,CNAME,SOA,TXT",
    help="Comma separated DNS records to query. Defaults to: A,MX,NS,CNAME,SOA,TXT",
)
@click.option(
    "--tor-routing",
    is_flag=True,
    help="Route HTTP traffic through Tor (uses port 9050)."
    " Slows total runtime significantly",
)
@click.option(
    "--proxy-list",
    help="Path to proxy list file that would be used for routing HTTP traffic."
...
@click.option(
    "-c",
    "--cookies",
    help="Comma separated cookies to add to the requests. "
...
@click.option(
    "--vulners-nmap-scan",
    is_flag=True,
    help="Perform an NmapVulners scan. "
    "Runs instead of the regular Nmap scan and is longer.",
)
@click.option(
    "--vulners-path",
    default=os.path.join(MY_PATH, "utils/misc/vulners.nse"),
